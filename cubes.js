...
class Cubes{
    constructor(vecs){
        this.cubes = null;
        this.colors = [];
        this.cubeMats=[ new THREE.MeshBasicMaterial({color:0xFFFFFF, transparent:true, opacity:.75, polygonOffset: true, polygonOffsetFactor: 1, polygonOffsetUnits: 1}),
            ...
            for (var v = 0; v<vecs.vard.length; v++) {
                for (var n = 0; n<vecs.ncsd.length; n++) {
                    for (var c = 0; c<vecs.canl.length; c++) {
                        ...
                        (isNaN(filter[3])) ? color=this.colors[i]: color=filter[3]; 
